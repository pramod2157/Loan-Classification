...
for (b in 1:B)
{
  ...
  logistic <- glm(NewTrain_Y~ ., data=NewTrain_X[sampledata,][,-1], family = 'binomial', control = list(maxit = 50))
...
pred <- ifelse(yhat>0.6, 'Y', 'N')
pred <- factor(pred)
summary(pred)
submission <- data.frame(Loan_ID = NewTest$Loan_ID, Loan_Status = pred)
write.csv(submission, 'F:/KaggleProjectsAndData/AnalyticsVidhya/NewLoanPrediction/Submissions/BaggedLogistic.csv', row.names=FALSE, quote = FALSE)
